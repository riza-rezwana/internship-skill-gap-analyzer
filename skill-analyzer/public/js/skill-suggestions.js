...
async function loadSuggestions() {
  ...
  try {
    const res = await fetch(`/api/skill-suggestions/${sid}${iid ? `?internshipId=${iid}` : ''}`);
...
async function startCourse(courseId) {
  const sid = parseInt(ss.studentId.value, 10);
  try {
    const res = await fetch(`/api/skill-suggestions/${courseId}/start`, {
...
async function completeCourse() {
  ...
  try {
    const res = await fetch(`/api/skill-suggestions/${pendingCompleteCourseId}/complete`, {
...
async function loadHistory() {
  ...
  try {
    const res = await fetch(`/api/skill-suggestions/history/${sid}`);
    const json = await res.json();
